...
usage_help="Usage: $0 [-t <env>_global/research/<version>] [-b <tf_backend_config_prefix>] [-g <global_resource_deploy_from_region>] [-r <tf_state_region>]"
usage() { echo "$usage_help" 1>&2; exit 1; }

# Default values if none provided
tag="dev_global/research/1.0"
...
terraform plan -destroy -var-file "environments/${ENV}/${TEAM}/variables.tfvars" ${REGION_TFVARS} -var "env=${ENV}" -var "region=${REGION}" -target ${TARGET_MODULE} -out=tfplan -compact-warnings